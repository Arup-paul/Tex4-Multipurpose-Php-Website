...
<div id="main-content" class="main-content">
		<div class="page-title">
				...
				<h1><i class="fa fa-home"></i> New News/Project </h1>
				...
				if (isset($_GET['edit'])) {
					...
					<form action="" method="POST" enctype="multipart/form-data">
        				<div class="form-group col-lg-6 ">
        					<input type="text" name="pname" class=" form-control " placeholder="Link Name">
        				</div>
        				<div class="form-group col-lg-6 ">
        					<input type="text" name="purl" class=" form-control " placeholder="http://domain.com">
        				</div>
            				...
            				<div class="form-group col-lg-12 col-sm-12 col-md-12 col-xs-12">
            				      <input type="submit" name="service_insert" class="upload" value="Insert Link">
            				</div>
            			</form>
					';
				}
			...
			<table class="table table-bordered">
				...
				';		
						...
						$we_care_main="SELECT * FROM footer_link ORDER BY id DESC";
						...
						while ($we_care_main2=mysql_fetch_assoc($we_care_main1)) { 
							...
							echo '
							 <tr>
								...
								<td>
								    <a href="manage_footer.php?delete='.$empid.'">DELETE</a>
								</td>
						      </tr>
							';
...
<?php
	...
	if(isset($_POST['service_insert'])){
		...
		$purl=mysql_real_escape_string($_POST['purl']);
		if(empty($pname) || empty($purl)){	
			
			echo "<script>alert('Failed!!! Please Fillup All Data');</script>";
		}else{
			$pro_query="INSERT INTO footer_link (title,url) 
			VALUES('$pname','$purl')";
	...
	if(isset($_GET['delete'])){
		$pid=$_GET['delete'];
		$del_query="DELETE FROM footer_link WHERE id='$pid'";
